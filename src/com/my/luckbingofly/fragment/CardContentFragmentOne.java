...
public class CardContentFragmentOne extends Fragment {
	...
	public static List<Checkbox> mCheckboxEtityList1;// 存放实体类的list----25个，
	private Checkbox checkbox;

	private int[] mCheckNum;// 产生的随机数
	private int countOfChecked = 1;// 用来记录已经被选中的checkbox的个数

	public static ArrayList<Integer> subscript1 = new ArrayList<Integer>();
	...
	private void initData() {
		...
		mCheckboxEtityList1 = new ArrayList<Checkbox>();

		for (int i = 0; i < mCheckNum.length; i++) {
			checkbox = new Checkbox();
			checkbox.setValue(mCheckNum[i]);
			mCheckboxEtityList1.add(checkbox);// 最后共存了25个对象
		}
	...
	private void initView() {
		...
		// 给每个checkbox设置监听。---同时设置要显示的值。----一次for循环，干了两件事。
		for (int i = 0; i < mCheckBoxsList.size(); i++) {
			if (i == 12) {
				mCheckboxEtityList1.get(i).setCheckState(true);
			...
			cbBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
				...
				public void onCheckedChanged(CompoundButton buttonView,
					...
					// TODO Auto-generated method stub
					if (isChecked) {
						Checked(position, cbBox, mCheckboxEtityList1);
					} else {
						notChecked(position, cbBox, mCheckboxEtityList1);
					}
				}
			});
			cbBox.setText("" + mCheckboxEtityList1.get(i).getValue());
		}
	}
	 ...
	 * @param cbBox
	 */
	public void Checked(int i, final CheckBox cbBox,
		...
		if (countOfChecked >= 5) {
			if (WinPrizeStateUtils.winPrize(checkboxEtityList)) {
				Toast.makeText(getActivity(), "baby,中奖了", Toast.LENGTH_SHORT)
						.show();
				subscript1 = WinPrizeStateUtils.winPrizeTwo(checkboxEtityList);
			}
		}
	}
	 ...
	 * @param cbBox
	 */
	public void notChecked(int i, final CheckBox cbBox,
			List<Checkbox> checkboxEtityList) {
