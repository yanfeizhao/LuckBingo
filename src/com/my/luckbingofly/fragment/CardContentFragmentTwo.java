...
public class CardContentFragmentTwo extends Fragment{
	...
	public static  List<Checkbox> mCheckboxEtityList2;// 存放实体类的list----25个，
	private Checkbox checkbox;

	private int[] mCheckNum;//产生的随机数
	private int countOfChecked = 1;//用来记录已经被选中的checkbox的个数
	
	public static ArrayList<Integer> subscript2=new ArrayList<Integer>();
	...
	private void initData() {
		...
		mCheckboxEtityList2 = new ArrayList<Checkbox>();

		for (int i = 0; i < mCheckNum.length; i++) {
			checkbox = new Checkbox();
			checkbox.setValue(mCheckNum[i]);
			mCheckboxEtityList2.add(checkbox);// 最后共存了25个对象
		}
	...
	private void initView() {
		...
		for (int i = 0; i < mCheckBoxsList.size(); i++) {
			if (i == 12) {
				mCheckboxEtityList2.get(i).setCheckState(true);
			...
			cbBox.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
				...
				public void onCheckedChanged(CompoundButton buttonView,
					...
					// TODO Auto-generated method stub
					if (isChecked) {
						Checked(position, cbBox,mCheckboxEtityList2);
					} else {
						notChecked(position, cbBox,mCheckboxEtityList2);
					}
				}
			});
			cbBox.setText("" + mCheckboxEtityList2.get(i).getValue());
		}
	}
	...
	public void Checked(int i, final CheckBox cbBox,List<Checkbox> checkboxEtityList) {
		...
		if (countOfChecked >= 5) {
			if (WinPrizeStateUtils.winPrize(checkboxEtityList)) {
				...
				subscript2=WinPrizeStateUtils.winPrizeTwo(checkboxEtityList);
//				Log.e("subscript", subscript2.size()+"");
			}
		}
	}
